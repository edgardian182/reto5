...
questions = {}
answers = {}
questionnaire = open('reto5_preguntas.txt')
i = 1
q = 1
a = 1
...
deck = Questions_Deck.new(questions)
...
while deck.questions.length > 0
  ...
  while user_input != answers[deck.present_question[0]] && attempt < 3
    puts "Incorrecto!, Trata de nuevo"
    puts "Adivinar: "
    user_input = gets.chomp.upcase
    attempt += 1
  end
  points += 1000 if attempt <= 3 && user_input == answers[deck.present_question[0]]
  puts "Correcto!" if user_input == answers[deck.present_question[0]]
  puts
end
